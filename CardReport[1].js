...
const CardReport = ({ transactions }) => {
  const cardBalances = transactions.reduce((acc, transaction) => {
    if (transaction.paymentMethod && transaction.paymentMethod !== 'Efectivo' && transaction.paymentMethod !== 'Transferencia') {
  ...
  return (
    ...
    >
      ...
      {!hasCardData && (
        ...
        >
          <p>No hay operaciones registradas con tarjetas de crédito.</p>
